...
for r in cur:
    ...
    for t in items:
        ...
        cd = g_clerks.setdefault(clerk, [{}])[0].setdefault(dt_i, [0, 0, 0, 0])
        cd[idx + 0] += ttl_price
        cd[idx + 1] += ttl_cost

        sd = g_sales[''][0].setdefault(dt_i, [0, 0, 0, 0])
        sd[idx + 0] += ttl_price
        sd[idx + 1] += ttl_cost
...
for sid,td in g_items.items():
    ...
    for f_k,f_v in td[1].items():
        n_l_ts.append( (f_k,map(lambda f_x: round(f_x, 2), f_v)) )
...
for sid,dd in g_depts.items():
    ...
    for f_k,f_v in dd[1].items():
        n_l_ds.append( (f_k,map(lambda f_x: round(f_x, 2), f_v)) )
...
for d in (g_types, g_clerks, g_sales):
    for cnz,cd in d.items():
        cd[0] = [ (f_k,map(lambda f_x: round(f_x, 2), f_v)) for f_k,f_v in cd[0].items() ]
        cd[0].sort(key=lambda f_x: f_x[0])

fnz = os.path.join(config.DATA_DIR, 'items_stat_v3.txt')
cPickle.dump({'cates': g_cates, 'depts': g_depts, 'types': g_types, 'items': g_items, 'clerks': g_clerks, 'sales': g_sales}, open(fnz, 'wb'), 1)
print "Done"
