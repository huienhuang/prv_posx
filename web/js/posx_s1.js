...
{
    var ctx = v_msgbox.data('ctx');
    ctx.header.text(title);
    if(msg_type)
        ctx.content.html(msg);
    else
        ctx.content.empty().append($('<div style="white-space:pre-wrap"></div>').text(msg));
    v_msgbox.modal('show');
}
...
{
    if (!v_msgbox) {
        v_msgbox = $('<div id="__dlg_msgbox" class="ui modal"><div class="header">pk</div><div class="content">pkk</div></div>')
        .appendTo($('body'))
        .modal({
            allowMultiple:true,
            modal:true,
    ...
    if(v_msgbox.modal('is active')) return;
